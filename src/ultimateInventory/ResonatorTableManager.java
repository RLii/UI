...
public class ResonatorTableManager {
    Connection connection;
    public ResonatorTableManager(Connection conn, boolean reset) throws SQLException {
        connection = conn;
        if(reset)
        {
            ...
            try
            {
                stmt.execute("DROP TABLE Resonators");
            }
            ...
            {
                stmt.execute("CREATE TABLE Resonators ("
    ...
    {
        String sql = "DELETE FROM Resonators WHERE ID = ?";
    ...
    public ObservableList<String> getIdNames() throws SQLException
    {
        ...
        ResultSet rs;
        Statement stmt = connection.createStatement();
        rs = stmt.executeQuery("SELECT ID FROM Resonators");
        while(rs.next())
        {
    ...
    {
        String sql = "SELECT * FROM Resonators WHERE ID = ?";
        PreparedStatement stmt = connection.prepareStatement(sql);
        stmt.setString(1,id);
    ...
    {
        String sql = "SELECT Currency FROM Resonators WHERE ID = ?";
        PreparedStatement stmt = connection.prepareStatement(sql);
        stmt.setString(1,id);
        ResultSet rs = stmt.executeQuery();
        rs.next();
