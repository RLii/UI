...
public class TipTableManager {
    Connection connection;
    public TipTableManager(Connection conn, boolean reset) throws SQLException {
        connection = conn;
        if(reset)
        {
            ...
            {
                stmt.execute("CREATE TABLE Tips ("
    ...
    {
        String sql = "INSERT INTO Tips(ID, VendorName, VendorEmail, VendorContact, VendorWebsite, Price, PayMethod, PayTerm, PurchaseDate, Currency, Note) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    {
        String sql ="UPDATE Tips SET VendorName = ?, VendorEmail = ?, VendorContact = ?, VendorWebsite = ?, Price = ?, PayMethod = ?, PayTerm = ?, PurchaseDate = ?, Currency = ?, Note = ? WHERE ID = ?";
        PreparedStatement prep = connection.prepareStatement(sql);
    ...
    public ObservableList<String> getIdNames() throws SQLException
    {
        ...
        ResultSet rs;
        Statement stmt = connection.createStatement();
        rs = stmt.executeQuery("SELECT ID FROM Tips");
        while(rs.next())
        {
    ...
    {
        String sql = "SELECT Price FROM Tips WHERE ID = ?";
        PreparedStatement stmt = connection.prepareStatement(sql);
        stmt.setString(1,id);
        ResultSet rs = stmt.executeQuery();
        rs.next();
    ...
    {
        String sql = "SELECT Currency FROM Tips WHERE ID = ?";
        PreparedStatement stmt = connection.prepareStatement(sql);
        stmt.setString(1,id);
        ResultSet rs = stmt.executeQuery();
        rs.next();
