...
public class PipeTableManager {
    Connection connection;
    public PipeTableManager(Connection conn, boolean reset) throws SQLException {
        connection = conn;
        if(reset)
        {
    ...
    {
        String sql = "INSERT INTO Pipes(ID, VendorName, VendorEmail, VendorContact, VendorWebsite, Price, PayMethod, PayTerm, PurchaseDate, Currency, Note) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    {
        String sql ="UPDATE Pipes SET VendorName = ?, VendorEmail = ?, VendorContact = ?, VendorWebsite = ?, Price = ?, PayMethod = ?, PayTerm = ?, PurchaseDate = ?, Currency = ?, Note = ? WHERE ID = ?";
        PreparedStatement prep = connection.prepareStatement(sql);
    ...
    {
        String sql = "DELETE FROM Pipes WHERE ID = ?";
    ...
    public ObservableList<String> getIdNames() throws SQLException
    {
        ...
        ResultSet rs;
        Statement stmt = connection.createStatement();
        rs = stmt.executeQuery("SELECT ID FROM Pipes");
        while(rs.next())
        {
    ...
    {
        String sql = "SELECT * FROM Pipes WHERE ID = ?";
        PreparedStatement stmt = connection.prepareStatement(sql);
        stmt.setString(1,id);
    ...
    {
        String sql = "SELECT Price FROM Pipes WHERE ID = ?";
        PreparedStatement stmt = connection.prepareStatement(sql);
        stmt.setString(1,id);
        ResultSet rs = stmt.executeQuery();
        rs.next();
    ...
    {
        String sql = "SELECT Currency FROM Pipes WHERE ID = ?";
        PreparedStatement stmt = connection.prepareStatement(sql);
        stmt.setString(1,id);
        ResultSet rs = stmt.executeQuery();
        rs.next();
