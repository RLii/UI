...
public class RubberTableManager {
    Connection connection;
    public RubberTableManager(Connection conn, boolean reset) throws SQLException {
        connection = conn;
        if(reset)
        {
            ...
            try
            {
                stmt.execute("DROP TABLE Rubbers");
            }
            ...
            {
                stmt.execute("CREATE TABLE Rubbers ("
    ...
    {
        String sql = "INSERT INTO Rubbers(ID, VendorName, VendorEmail, VendorContact, VendorWebsite, Price, PayMethod, PayTerm, PurchaseDate, Currency, Note) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    {
        String sql ="UPDATE Rubbers SET VendorName = ?, VendorEmail = ?, VendorContact = ?, VendorWebsite = ?, Price = ?, PayMethod = ?, PayTerm = ?, PurchaseDate = ?, Currency = ?, Note = ? WHERE ID = ?";
        PreparedStatement prep = connection.prepareStatement(sql);
    ...
    {
        String sql = "DELETE FROM Rubbers WHERE ID = ?";
    ...
    public ObservableList<String> getIdNames() throws SQLException
    {
        ...
        ResultSet rs;
        Statement stmt = connection.createStatement();
        rs = stmt.executeQuery("SELECT ID FROM Rubbers");
        while(rs.next())
        {
    ...
    {
        String sql = "SELECT * FROM Rubbers WHERE ID = ?";
        PreparedStatement stmt = connection.prepareStatement(sql);
        stmt.setString(1,id);
    ...
    {
        String sql = "SELECT Price FROM Rubbers WHERE ID = ?";
        PreparedStatement stmt = connection.prepareStatement(sql);
        stmt.setString(1,id);
        ResultSet rs = stmt.executeQuery();
        rs.next();
    ...
    {
        String sql = "SELECT Currency FROM Rubbers WHERE ID = ?";
        PreparedStatement stmt = connection.prepareStatement(sql);
        stmt.setString(1,id);
        ResultSet rs = stmt.executeQuery();
        rs.next();
