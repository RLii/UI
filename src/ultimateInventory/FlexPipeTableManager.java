...
public class FlexPipeTableManager {
    Connection connection;
    public FlexPipeTableManager(Connection conn, boolean reset) throws SQLException {
        connection = conn;
        if(reset)
        {
            ...
            try
            {
                stmt.execute("DROP TABLE FlexPipes");
            }
            ...
            {
                stmt.execute("CREATE TABLE FlexPipes ("
    ...
    {
        String sql = "INSERT INTO FlexPipes(ID, VendorName, VendorEmail, VendorContact, VendorWebsite, Price, PayMethod, PayTerm, PurchaseDate, Currency, Note) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    {
        String sql ="UPDATE FlexPipes SET VendorName = ?, VendorEmail = ?, VendorContact = ?, VendorWebsite = ?, Price = ?, PayMethod = ?, PayTerm = ?, PurchaseDate = ?, Currency = ?, Note = ? WHERE ID = ?";
        PreparedStatement prep = connection.prepareStatement(sql);
    ...
    {
        String sql = "DELETE FROM FlexPipes WHERE ID = ?";
    ...
    public ObservableList<String> getIdNames() throws SQLException
    {
        ...
        ResultSet rs;
        Statement stmt = connection.createStatement();
        rs = stmt.executeQuery("SELECT ID FROM FlexPipes");
        while(rs.next())
        {
    ...
    {
        String sql = "SELECT * FROM FlexPipes WHERE ID = ?";
        PreparedStatement stmt = connection.prepareStatement(sql);
        stmt.setString(1,id);
    ...
    {
        String sql = "SELECT Price FROM FlexPipes WHERE ID = ?";
        PreparedStatement stmt = connection.prepareStatement(sql);
        stmt.setString(1,id);
        ResultSet rs = stmt.executeQuery();
        rs.next();
    ...
    {
        String sql = "SELECT Currency FROM FlexPipes WHERE ID = ?";
        PreparedStatement stmt = connection.prepareStatement(sql);
        stmt.setString(1,id);
        ResultSet rs = stmt.executeQuery();
        rs.next();
