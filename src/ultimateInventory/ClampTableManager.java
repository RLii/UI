...
public class ClampTableManager {
    Connection connection;
    public ClampTableManager(Connection conn, boolean reset) throws SQLException {
        connection = conn;
        if(reset)
        {
            ...
            try
            {
                stmt.execute("DROP TABLE Clamps");
            }
    ...
    {
        String sql = "INSERT INTO Clamps(ID, VendorName, VendorEmail, VendorContact, VendorWebsite, Price, PayMethod, PayTerm, PurchaseDate, Currency, Note) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    {
        String sql ="UPDATE Clamps SET VendorName = ?, VendorEmail = ?, VendorContact = ?, VendorWebsite = ?, Price = ?, PayMethod = ?, PayTerm = ?, PurchaseDate = ?, Currency = ?, Note = ? WHERE ID = ?";
        PreparedStatement prep = connection.prepareStatement(sql);
    ...
    {
        String sql = "DELETE FROM Clamps WHERE ID = ?";
    ...
    {
        String sql = "SELECT * FROM Clamps WHERE ID = ?";
        PreparedStatement stmt = connection.prepareStatement(sql);
        stmt.setString(1,id);
    ...
    {
        String sql = "SELECT Price FROM Clamps WHERE ID = ?";
        PreparedStatement stmt = connection.prepareStatement(sql);
        stmt.setString(1,id);
        ResultSet rs = stmt.executeQuery();
        rs.next();
    ...
    {
        String sql = "SELECT Currency FROM Clamps WHERE ID = ?";
        PreparedStatement stmt = connection.prepareStatement(sql);
        stmt.setString(1,id);
        ResultSet rs = stmt.executeQuery();
        rs.next();
