...
public class BoltTableManager {
    Connection connection;
    public BoltTableManager(Connection conn, boolean reset) throws SQLException {
        connection = conn;
        if(reset)
        {
            ...
            {
                stmt.execute("CREATE TABLE Bolts ("
    ...
    {
        String sql = "INSERT INTO Bolts(ID, VendorName, VendorEmail, VendorContact, VendorWebsite, Price, PayMethod, PayTerm, PurchaseDate, Currency, Note) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    {
        String sql ="UPDATE Bolts SET VendorName = ?, VendorEmail = ?, VendorContact = ?, VendorWebsite = ?, Price = ?, PayMethod = ?, PayTerm = ?, PurchaseDate = ?, Currency = ?, Note = ? WHERE ID = ?";
        PreparedStatement prep = connection.prepareStatement(sql);
    ...
    {
        String sql = "DELETE FROM Bolts WHERE ID = ?";
    ...
    public ObservableList<String> getIdNames() throws SQLException
    {
        ...
        ResultSet rs;
        Statement stmt = connection.createStatement();
        rs = stmt.executeQuery("SELECT ID FROM Bolts");
        while(rs.next())
        {
    ...
    {
        String sql = "SELECT * FROM Bolts WHERE ID = ?";
        PreparedStatement stmt = connection.prepareStatement(sql);
        stmt.setString(1,id);
    ...
    {
        String sql = "SELECT Price FROM Bolts WHERE ID = ?";
        PreparedStatement stmt = connection.prepareStatement(sql);
        stmt.setString(1,id);
        ResultSet rs = stmt.executeQuery();
        rs.next();
    ...
    {
        String sql = "SELECT Currency FROM Bolts WHERE ID = ?";
        PreparedStatement stmt = connection.prepareStatement(sql);
        stmt.setString(1,id);
        ResultSet rs = stmt.executeQuery();
        rs.next();
